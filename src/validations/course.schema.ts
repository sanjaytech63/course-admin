...
const optionalNumberString = z.string().optional().transform((val, ctx) => {
...
export const courseFormSchema = z.object({
  ...
  discountedPrice: optionalNumberString
    .refine(val => val === undefined || val >= 0, 'Discounted price cannot be negative')
    .nullable()
    .optional(),

  discountPercentage: optionalNumberString
    .refine(val => val === undefined || (val >= 0 && val <= 100), 'Discount percentage must be between 0 and 100')
    .nullable()
    .optional(),
  ...
  reviewCount: numberString
    .refine(val => val >= 0, 'Review count cannot be negative'),

  students: numberString
    .refine(val => val >= 0, 'Students count cannot be negative'),

  totalHours: numberString
    .refine(val => val >= 0, 'Total hours cannot be negative'),

  lectures: numberString
    .refine(val => val >= 0, 'Lectures count cannot be negative'),
