...
interface UseSubscribersReturn {
    ...
    fetchSubscribers: (search?: string, page?: number, limit?: number) => Promise<void>;
...
export const useSubscribers = (): UseSubscribersReturn => {
    ...
    const fetchSubscribers = async (search: string = '', page: number = pagination.page, limit: number = pagination.limit) => {
        setLoading(true);
        setError(null);
        try {
    ...
    const unsubscribeUser = async (email: string, id: string): Promise<void> => {
        try {
            ...
            setSubscribers(prev =>
                prev.map(sub =>
                    sub._id === id
                        ? { ...sub, status: 'inactive', unsubscribedAt: new Date().toISOString() }
                        : sub
                )
            );
    ...
    const bulkUnsubscribe = async (ids: string[]): Promise<void> => {
        try {
            const unsubscribePromises = ids.map(id => {
                ...
                return subscriber ? subscribeApi.unsubscribeUser(subscriber.email) : Promise.resolve();
            });
