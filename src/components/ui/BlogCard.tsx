...
import { FiEdit2, FiTrash2, FiEye, FiClock, FiUser, FiCalendar } from 'react-icons/fi';
...
const BlogCard: React.FC<BlogCardProps> = ({ blog, onEdit, onDelete, onView }) => {
    ...
    return (
        ...
        >
            ...
            {blog.badge && (
                <div className="mb-3">
                    <span className={`inline-block px-3 py-1 text-xs font-semibold rounded-full border ${getBadgeColor(blog.badge)}`}>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pt-4 border-t border-gray-100">
                <div className="flex items-center text-sm">
                    <div className={`flex items-center space-x-1 px-2 py-1 rounded-full text-xs ${blog.isPublished
                        ? 'bg-green-100 text-green-700'
                        : 'bg-gray-100 text-gray-600'
                        }`}>
                        <div className={`w-2 h-2 rounded-full ${blog.isPublished ? 'bg-green-500' : 'bg-gray-400'
                            }`}></div>
