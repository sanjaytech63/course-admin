        ...
        (set, get) => ({
            ...
            setFormData: (formData) =>
                set((state) => ({
                    ...
                    errors: {
                        ...state.errors, ...Object.keys(formData).reduce((acc, key) => ({
                            ...acc, [key]: ''
                        }), {})
                    }
                })),
            ...
            updateUser: async (e: React.FormEvent) => {
                e.preventDefault();
                const { formData, setUserLoading, setErrors, resetForm, fetchUsers } = get();
                    ...
                    if (err instanceof Error && 'issues' in err) {
                        ...
                        if (Array.isArray(zodError.issues)) {
                            zodError.issues.forEach((issue: any) => {
                                if (issue.path?.[0]) {
                                    fieldErrors[issue.path[0] as keyof UserErrors] = issue.message;
                                }
                            });
                        }
